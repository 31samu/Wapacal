...
Under **Appearance**, choose Forest, Neutral, Ocean, Plum, Rose, Sand, or Custom in **Color theme**. Each theme has light and dark variants. Neutral uses a grayscale palette. Like every theme, it can respect calendar colors; turn off **Calendar colors** for a fully grayscale wallpaper. Custom lets you pick background, text, and accent colors separately for light and dark appearance; grid lines and highlights follow those colors. Turn off **Calendar colors** to use the theme for events as well. Custom starts with Neutral’s colors until you save your own. Custom keeps calendar labels and grid lines visible against the background. Text inside event boxes uses your exact selected text color for each appearance. Custom colors stay saved when you try another preset.
